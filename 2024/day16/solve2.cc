// Similar to solve2.cc, but instead of using std::priority_queue<> as the queue,
// uses a vector of vectors, where the indices of the outer vector are the
// distances to solve, and the inner vectors the states for each distance.
//
// This reduces the time complexity from O(n log n) where n is the number of
// states visited to O(n + d) where d is the maximum distance visited. In my
// large testcases, n is a few orders of magnitude larger than d, so this is
// practically O(n).
